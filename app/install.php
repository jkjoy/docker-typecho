...
if (!file_exists(dirname(__FILE__) . '/data/config.inc.php')) {
    ...
    define('__TYPECHO_THEME_DIR__', '/data/themes');
...
";
    ...
    if (!$return) {
        $configWritten = @file_put_contents(__TYPECHO_ROOT_DIR__ . '/data/config.inc.php', $code) !== false;
    }
...
function install_remove_config_file()
{
    ...
    if ($configWritten) {
        unlink(__TYPECHO_ROOT_DIR__ . '/data/config.inc.php');
    }
}
...
function install_check(string $type): bool
{
    switch ($type) {
        case 'config':
            return file_exists(__TYPECHO_ROOT_DIR__ . '/data/config.inc.php');
        case 'db_structure':
        case 'db_data':
            global $installDb;
...
function install_step_2_perform()
{
    ...
    $configMap = [
        ...
        'SQLite' => [
            'dbFile' => __TYPECHO_ROOT_DIR__ . '/data/' . uniqid() . '.db'
        ]
    ];
    ...
    if (isset($dbConfig['file']) && preg_match("/^[a-z0-9]+\.[a-z0-9]{2,}$/i", $dbConfig['file'])) {
        $dbConfig['file'] = __DIR__ . '/data/' . $dbConfig['file'];
    }
